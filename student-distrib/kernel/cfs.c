...
// static thread_t *pick_next_task(sched_t *curr, int8_t wakeup);
// static sched_t *pick_next_entity(void);
// static void put_prev_task(sched_t *prev, int8_t wakeup);
...
//     task->state = SLEEPING;
//     list_add_tail(&task->wait_node, wait_queue);
//     __schedule(task);
// }
...
//  * @param wakeup : has the task just waken up?
